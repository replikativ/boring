...
(defn -main [& args]
  (let [only (set args)
    ...
    ;; The tier-matched boring-vs-hako table (T1/T2/T3, time + heap alloc),
    ;; reusing BOTH sides -- the one PERFORMANCE.md quotes for the reuse tier.
    ;; `ab` above pairs boring-reused against hako-per-call; this does not.
    (when (run? :hako-ab)
